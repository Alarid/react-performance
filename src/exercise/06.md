...
State colocation is placing state as close to where it's relevant as possible.

It's a good practice to try and seperate context as much as possible, so a
context only contains domain-specific state. This way we don't impact the whole
app by updating a global state only used in a part of our app. (component
consuming the state of a context re-render when it changes)

A higher-order component (HOC) is a function that takes a component and returns
a new component. In this exercise (extra 3) we build one to abstract an
optimization pattern we are applying to the app: make a memoized component take
as prop only a slice of the state, so it will only re-render if the slice he is
interested in changed.

Recoil is really powerfull when it comes to optimize an UI with thousands of
components sharing a context state, for example. It splits the state into atoms,
which make the state changes for individual components super fast. It should not
be used all the time and all over the place though, as it adds complexity and
maintenance needs to the codebase. It's a good choice when building complex and
rich UIs with frequent state updates impacting a lot of components.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Performance%20%E2%9A%A1&e=06%3A%20Fix%20%22perf%20death%20by%20a%20thousand%20cuts%22&em=yoh.legrand%40gmail.com).
