...
Lazy loading means loading something (a component) **after** the page, right
when the user needs it, to make the page load faster.

In React, this is done by importing the component with `React.lazy` and
rendering it inside a `<React.Suspense>` wrapper. Suspense will load the
component on demand, rendering a fallback component for the loading state.

```js
const MyCompnent = React.lazy(() => import('./MyComponent'))
...
Eager loading is lazy loading the component **in advance** when we think the
user is gonna need it (for example, on a mouse over of focus). Then, combined
with `React.lazy` and `React.Suspense`, webpack will just load it _once_ and
render it when a condition is fullfilled. This is usefull when the code takes
some time to load: we can start the loading in advance.

We can ask the browser to start pre-fetching lazy imports that we are confident
the user is gonna need, using a special comment in the import:

```js
const loadGlobe = () => import(/* webpackPrefetch: true */ '../globe')
```

The browser will load the module as soon as the page is loaded and he's not busy
doing something else. There's a bunch of other usefull
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Performance%20%E2%9A%A1&e=01%3A%20Code%20splitting&em=yoh.legrand%40gmail.com).
