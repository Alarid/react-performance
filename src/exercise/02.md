...
The performance tab of the Chrome dev tools can help detect expensive
computations that might be optimised with `React.useMemo`, if the computation
itself doesn't need to be triggered on each render but only when some inputs
change.

It's better to profile the app in production mode, because React has a lot of
stuff going on in development mode.

Web workers can help reduce the amount of computations the browser has to do
when performing expensive tasks. A loader like `workerize-loader` can turn any
module into a web worker, turning its functions into async functions as a direct
effect.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Performance%20%E2%9A%A1&e=02%3A%20useMemo%20for%20expensive%20calculations&em=yoh.legrand%40gmail.com).
