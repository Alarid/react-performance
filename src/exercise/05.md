...
When the value of a context changes, or when the context re-render, it
re-renders all components that consume this value. This is usually not that much
of a problem, but when working with heavy UIs that are still running slow after
applying all basic optimizations (like memoization of the component with
`React.memo`), the main solution consists in memoizing the context value with
`React.useMemo`.

This way, when the context re-render, its value didn't change from the previous
render, so the consumers do not get re-rendered.

Another solution is to define 2 context: one for the state, and one for the
update of the state. This way, when the state changes, component that only
consume the updater (dispatch) will not get re-render
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Performance%20%E2%9A%A1&e=05%3A%20Optimize%20context%20value&em=yoh.legrand%40gmail.com).
