...
Note: it's better to start by fixing slow renders first, rather than trying to
reduce unnecessary re-renders.

DOM updates are optimized with React: when a component renders, it create React
elements with `React.createElement`. Then, React compares those elements with
the previous ones (_reconciliation_), and updates the DOM for the elements that
are different (_comit_) if needed.

`React.memo` is a way to tell React not to re-render a component when it's
parent re-render. It shouldn't be used all the time because it can slow down the
app and it makes the code more complex. But it some cases, it can be usefull.

`React.memo` accepts a second argument, which is custom compare function. It
takes the previous props and the next props as arguments, and should returns
true when it is **unnecessary** to re-render the component. Although, when
possible, it's better to use primitive values as props for our component, so
`React.memo` can determine by itself if the component should or should not
re-render.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Performance%20%E2%9A%A1&e=03%3A%20React.memo%20for%20reducing%20unnecessary%20re-renders&em=yoh.legrand%40gmail.com).
