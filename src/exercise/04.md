...
When it comes to rendering a huge number of elements on the page, the browser
can begin to struggle handling the thousands of DOM updates made by React during
the comit phase. This is were `react-virtual` comes in handy: it is a library
that allow us to only render a "window" of those elements, the ones that the
user is actually seing. Everything else is not rendered. We can configure
`react-virtual` to render a few elements outside of that window, so that the
user doesn't have to wait if he scrolls too fast.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Performance%20%E2%9A%A1&e=04%3A%20Window%20large%20lists%20with%20react-virtual&em=yoh.legrand%40gmail.com).
